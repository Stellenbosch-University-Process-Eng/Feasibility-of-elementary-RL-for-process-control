%% Script that may be used to calculate the IAE responses generated by training
%% a SARSA agent for different discretization fineness settings.
%% Notes:
% The generated data and its processing is given in the Excel spreadsheet
% named "Experimental result summary". 
% Need to load a predefined set of arrays (the action-value hypervolumes)
% The structure A contains the fields "out_Reps" that may be indexed and 
% contains all the hypervolumes.  Each hypervolume is denoted by "action_value".
% seed random number generator to ensure exploration-exploitation strategy 
% is repeatable
clc
% clearvars -except A B ans
tic
%%
%% 
% seed random number generator to ensure exploration-exploitation strategy 
...
training.nmberOfEps = 11500;                 % number of episodes to use in training
...
%% settings for sampling function
...
if sumTables == 1
    ...
    for i = 1:1:training.nmberOfEps
        Reps.action_value = Reps.action_value + A.out_Reps(i).action_value;
        fprintf('\n%.0f\n', i)
    end
    
elseif sumTables ~= 1
    
    Reps.action_value = run_16_overall_Q.Reps;
    
end

sample_Size = 100; % sample size for IAE data generation
%% run trained sarsa-LPF controller
for sim_episodeCntr = 1:1:sample_Size
    par.epsilonVec = 0.001*ones(size(dStates,1),1);
    ...
    %% store IAE information
    IAE_data(sim_episodeCntr) = learning_sim.agentExp{1,sim_episodeCntr}{end,9};
    
end % end of episode loop

subplot(2,1,1)
plot(cell2mat(sim_learning(:,1)) +  cell2mat(learning_sim.SP{1, 1}),'LineWidth', 2)
title('SP tracking behaviour'); ylabel('CB (mol/L)');
xlabel('discrete time steps (unscaled)');
hold on
plot(cell2mat(learning_sim.SP{1, 1}),'LineWidth', 2)
hold on
yyaxis right
plot(cell2mat(learning_sim.DV{1, 1}),'k--')
legend('CB(mol/L)','CB SP (mol/L)','DV (inlet temperature (degC))')
set(gca, 'fontsize', 14)

subplot(2,1,2)
plot(cell2mat(learning_sim.discreteSelections{1, 1}(:,3)),'ko')
hold on
plot(cell2mat(sim_learning(:,5)),'LineWidth', 2); xlabel('discrete time steps (unscaled)');
ylabel('normalized inlet flowrate (1/h)')
set(gca, 'fontsize', 14)
