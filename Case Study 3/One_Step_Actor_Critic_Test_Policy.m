...
% Name: AC_PSE_MFS.m
%% do not clear command window and workspace
%clc
%clear
%% 
tic
...
policyVarianceVector = repmat(0.1*ones(1,numS),(numS*numS*par.numA));
...
%%
par.gamma = 0.99;%0.9;           % Discount rate
...
training.firstPSEVec = [(66.84/100)]';%[(66.8/100)]'; % first CV value
...
rvTwo.lower_SP = 0.58;%0.68;
rvTwo.upper_SP = 0.66;%0.77;

%% lower and upper bounds for DV sampling (rock fraction fed to circuit)
rvTwo.lower_DV = 0.465 - 0.15;%0.465 - 0.15;
rvTwo.upper_DV = 0.465 + 0.10;%0.465;
...
rvOne.lower_DV = 5;%6.03;%5;
rvOne.upper_DV = 7;%6.03;%6.8;
...
%% lower and upper bounds for SP sampling (PSE)
rv_test_Two.lower_SP = 0.58;%0.68;
rv_test_Two.upper_SP = 0.66;%0.77;

%% lower and upper bounds for DV sampling (rock fraction fed to circuit)
rv_test_Two.lower_DV = 0.465 - 0.15;%0.465 - 0.15;
rv_test_Two.upper_DV = 0.465 + 0.10;%0.465;
...
%% lower and upper bounds for DV sampling (rock hardness)
rv_test_One.lower_DV = 5;%5;
rv_test_One.upper_DV = 7;%6.8;
...
maxMFB = 8;%10;%20.484;%21.6;%100;%50;
...
par.manualDVOne_times = [10,20,30,40,50];%linspace(10,100,5);%[20,40];%% % times for rock hardness manual steps
par.manualDVTwo_steps = (0.465-0.15) + (0.465-(0.465-0.15))*rand(1,5);%[0.465,0.465]; %[0.348298975663461,0.445609845926606,0.346007873300914,0.452791636190688,0.388261678319224];%% % ore rock fraction manual steps
par.manualDVTwo_times = linspace(0,100,5);%[30,50]; %  % times for rock fraction manual steps

%%
% par.manualDVOne_steps = [6.03,6.03,6.03,6.03,6.03];%[4.5,5,6,7,4];%4.5 + (7-4.5)*rand(1,5);%[6.03,6.03];%%% rock hardness manual steps
% par.manualDVOne_times = [10,20,30,40,50];%linspace(10,100,5);%[20,40];%% % times for rock hardness manual steps
% par.manualDVTwo_steps = 0.465*ones(1,5);%(0.465-0.15) + (0.465-(0.465-0.15))*rand(1,5);%[0.465,0.465];%% % ore rock fraction manual steps
% par.manualDVTwo_times = linspace(0,100,5);%[30,50]; %  % times for rock fraction manual steps
...
par.manualSP_times = 10:10:100;%linspace(10,60,5); % times for PSE SP steps
...
for episodeCntr = 1:training.nmberOfEps
    if episodeCntr >= 2    % perform computations serially
    Reps = out_Reps(episodeCntr-1);
    end
...
%% display selected results of controller test
subplot(2,1,1)
%yyaxis left
% PSE (-)
...
legend('PSE','PSE SP')
plot((1:1:size(cell2mat(out_learning.agentExp{1, 1}(:,3))))/1, cell2mat(out_learning.agentExp{1, 1}(:,3)) + par.bandwidth);
plot((1:1:size(cell2mat(out_learning.agentExp{1, 1}(:,3))))/1, cell2mat(out_learning.agentExp{1, 1}(:,3)) - par.bandwidth);
%% unfiltered and filtered actions
subplot(2,1,2)
%yyaxis right
plot((1:1:size(cell2mat(out_learning.discreteSelections{1, 1})))/1,cell2mat(out_learning.discreteSelections{1, 1}),'rs','LineWidth',2)
%title('actions (filtered and unfiltered)'); 
xlabel('time (h)')
ylabel('MFS (t/h)')
hold on
plot((1:1:size(cell2mat(out_learning.agentExp{1, 1}(:,4))))/1,cell2mat(out_learning.agentExp{1, 1}(:,4)),'ko','LineWidth',2)
hold on

legend('unfiltered actions', 'filtered actions'); 

figure
plot((1:1:size(cell2mat(out_learning.discreteSelections{1, 1})))/1,cell2mat(out_learning.discreteSelections{1, 1}),'rs','LineWidth',2)
title('only unfiltered actions')
toc
...
%                     myEnvironment);    
                ...
                %%
                %%
%                 learning.nxtAgentAction = selectAction(Reps,par,...
%                                           learning.nxtAgentState_1,...
%                                           learning.nxtAgentState_2,...
%                                           learning.nxtAgentState_3,myEnvironment);
                                % obtain policy pmf vector at current state 
...
%     agentState_1 = state_1;
%     agentState_2 = state_2;
%     agentState_3 = state_3;
...
function [policyState_1_Centers,policyState_2_Centers,policyState_3_Centers,policy_MV_Centers] = Critic_RBF_placement(numS,error_low_bound,error_high_bound,rockfrac_low_bound,rockfrack_high_bound,PSE_SP_low_bound,PSE_SP_high_bound)
    ...
    REPEAT_state_2(1) = [];
    elementsForRepetition_3 = linspace(PSE_SP_low_bound,PSE_SP_high_bound,numS);
    policyState_2_Centers = repmat(REPEAT_state_2,1,(numS*1));%repmat(linspace(0,1,3),1,(3*3*5));
    for k = 1:1:size(elementsForRepetition_3,2)
        repeatingUnit_state_3(k,:) = elementsForRepetition_3(k).*ones(size(REPEAT_state_2,2),1)';
    end
    REPEAT_state_3 = 0;
...
%     subplot(1,2,1)
%     plot3(policyState_1_Centers,policyState_2_Centers,policyState_3_Centers,'ko','LineWidth',2)
%     xlabel('E(t)');ylabel('\alpha_r');zlabel('PSE SP'); grid on; axis tight

    %% old MV grid values
    % policy_MV_Centers = repmat(linspace(1,numA,numS),1,(numS*numS*numA));
...
%     subplot(1,2,2)
%     plot3(policyState_1_Centers,policyState_2_Centers,policy_MV_Centers,'ko','LineWidth',2)
%     xlabel('E(T)');ylabel('\alpha_r'),zlabel('actions'); grid on; axis tight
end
