...
training.nmberOfEps = 1000;          % number of episodes to use in training
...
par.MFB = @(t) 5.69; % step in feed balls to circuit (t/h)
%% new MFB
...
%% Use a slightly different dActions discretization to allow selections outside of bounds
%% without compromising the behavioural policy's control behaviour
...
par.manualDVOne_times = 100*[10,20,30,40,50];   % times for rock hardness manual steps
par.manualDVTwo_steps = (0.465-0.15) + (0.465-(0.465-0.15))*rand(1,5);  % ore rock fraction manual steps
par.manualDVTwo_times = 100*linspace(0,100,5);  % times for rock fraction manual steps
...
par.manualStepFlag = 0;%1; % flag for selecting manual steps in DVs and SP
...
function [learning,Reps] = train_agent(par,training,rvTwo,rvOne,...
        ...
        for stepCntr = 1:1:training.nmberOfSteps
            ...
            if State(currentTimeStamp) ~= myEnvironment.Terminal &&...
                ...
                %% map actions to Q-learning agent
                if currentTimeStamp == 1
                  [learning.crntAgentAction,learning.nxtAgentAction] = mapActionsToAgent(Action(currentTimeStamp),Action(currentTimeStamp),dActions);       

                elseif currentTimeStamp ~= 1
                  [learning.crntAgentAction,learning.nxtAgentAction] = mapActionsToAgent(Action(currentTimeStamp - 1),Action(currentTimeStamp),dActions);       
                end
                ...
                learning.hardness_Data{1,1}{stepCntr,1} = disturbanceValue_1;
                            ...
                            %% determine greedy action-value for Q-learning
                greedyQ = argMaxCalculation(learning.nxtAgentState_1,...
...
%% mapping of behavioural policy actions to coded actions of agent
function [crntAgentAction,nxtAgentAction] = mapActionsToAgent(crntAction,nxtAction,dActions)
    %% saturate valve data to ensure no training data points are negative
    crntAction = max(crntAction,min(dActions));
    nxtAction = max(nxtAction,min(dActions));
    crntAction = min(crntAction,max(dActions));
    nxtAction = min(nxtAction,max(dActions));
    % map current action to agent
    for cntr = 1:1:(size(dActions,1)-1)
        if crntAction >= dActions(cntr,1) && crntAction <= dActions(cntr+1,1)
            crntAgentAction = cntr;
        else
        end
    end
    % map next action to agent
    for cntr = 1:1:(size(dActions,1)-1)
        if nxtAction >= dActions(cntr,1) && nxtAction <= dActions(cntr+1,1)
            nxtAgentAction = cntr;
        else
        end
    end
...
%% determine the max of Q(S',a)
function greedyQ = argMaxCalculation(state_1,state_2,state_3,Reps,myEnvironment)
       vec = Reps.action_value(state_1,state_2,state_3,:);
       index = find( ismember( vec(:),max( vec(:) ) ) );
       [~,~,~,D] = ind2sub( size( vec(:,:,:,:) ), index );

       if size(D,1) > 1
           D = randi(myEnvironment.numberOfActions);
       end
       
       greedyQ = Reps.action_value(state_1,state_2,state_3,D);
end