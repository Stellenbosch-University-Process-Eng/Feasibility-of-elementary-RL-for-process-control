...
numS = 5;%4;%3;%4;%6;%3; % number of state components in each direction
par.numA = 5; % number of action components in each direction
error_low_bound = -1;%-0.1; % lower bound for error RBF placement
error_high_bound = 1;%0.1; % upper bound for error RBF placement
rockfrac_low_bound = -1;%0.315; % lower bound for rock fraction
rockfrack_high_bound = 1;%0.565; % upper bound for rock fraction
PSE_SP_low_bound = -1;%0.58; % lower bound for PSE SP
PSE_SP_high_bound = 1;%0.66;% upper bound for PSE SP
...
uniformCriticVariance_critic = 0.1;
...
%% Comment if training further starting with predefined critic and actor parameters
...
training.nmberOfEps = 176;             % number of episodes to use in training
        ...
        for stepCntr = 1:1:training.nmberOfSteps
            ...
            if State(currentTimeStamp) ~= myEnvironment.Terminal &&...
                ...
                elseif learning.nxtState_1 == myEnvironment.Terminal
                    ...
                    %% NB NOTE 2021-04-20: Update is only done until (total steps - 1). 
                    % Therefore, the non-terminal update is used throughout.
                    Reps.Theta = Reps.Theta + par.alphaActor*discountI*...
                                (sigma-Reps.Weights'*PHI)*eligibilityVec;
