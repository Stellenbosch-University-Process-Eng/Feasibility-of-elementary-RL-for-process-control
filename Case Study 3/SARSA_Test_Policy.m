%% short description
% SARSA control applied to the Hulbert model
% State-action space for the agent defined as follows:
...
training.nmberOfEps = 5000;  % number of episodes to use in training
training.nmberOfSteps = 100; % number of steps allowed per episode 
...
numError_intervals = 5; % feedback error
...
dActions = linspace(33,66,actionResAvailable)';%linspace(33,60,actionResAvailable)';%linspace(35, 66, actionResAvailable)'; %linspace(30,85,actionResAvailable)';
...
for sim_episodeCntr = 1:1
        par.epsilonVec = 0.001.*par.epsilonVec;
        [learning_sim,sim_Reps] = train_agent(par,training,rv_test_Two,rv_test_One,learning,...
                                    dStates,dActions,setPoint,myEnvironment,...
                                    Reps,startingCoordinate,sim_episodeCntr,filter);
        sim_Reps(sim_episodeCntr) = sim_Reps;
        sim_learning(sim_episodeCntr) = learning_sim;
end % end of episode loop

%% report control statistics
STATS.IAE = cell2mat(learning_sim.agentExp{1, 1}(end,7));
STATS.ITAE = cell2mat(learning_sim.agentExp{1, 1}(end,8));
STATS.TV = cell2mat(learning_sim.agentExp{1, 1}(end,9));
...
%yyaxis left
% PSE (-)
plot((1:1:size(cell2mat(sim_learning.agentExp{1, 1}(:,1))))/1,(cell2mat(sim_learning.agentExp{1, 1}(:,1))/-1) + ...
    cell2mat(sim_learning.agentExp{1, 1}(:,3)),'-.b', 'LineWidth', 1);
% title('Control error for PSE'); 
xlabel('time (h)');
ylabel('PSE (-)'); grid on; hold on

%% PSE SP
plot((1:1:size(cell2mat(sim_learning.agentExp{1, 1}(:,3))))/1,cell2mat(sim_learning.agentExp{1, 1}(:,3)),'k','LineWidth', 2); hold on
legend('PSE','PSE SP')
%% unfiltered and filtered actions
subplot(3,1,2)
%yyaxis right
plot((1:1:size(cell2mat(sim_learning.discreteSelections{1, 1})))/1,cell2mat(sim_learning.discreteSelections{1, 1}),'gs')
%title('actions (filtered and unfiltered)'); 
xlabel('time (h)')
ylabel('MFS (t/h)')
hold on
plot((1:1:size(cell2mat(sim_learning.agentExp{1, 1}(:,4))))/1,cell2mat(sim_learning.agentExp{1, 1}(:,4)),'ko')
hold on

legend('unfiltered actions', 'filtered actions'); 
% %% rock fraction in ore
subplot(3,1,3)
% during test
plot((1:1:size(cell2mat(sim_learning.agentExp{1, 1}(:,2))))/1,cell2mat(sim_learning.agentExp{1, 1}(:,2)), '--b', 'LineWidth', 2); hold on
% nominal
plot((1:1:size((cell2mat(sim_learning.agentExp{1, 1}(:,2)))))/1,0.465.*ones(size(cell2mat(sim_learning.agentExp{1, 1}(:,2)))), 'go'); 
hold on
xlabel('time (h)'); ylabel('ore rock fraction (-)')

%% hardness of ore
yyaxis right
crntAxis = gca;
crntAxis.YColor = 'blue';
ylabel('ore hardness (rock abrasion factor in kWh/t)')

%set(h,'YColor', 'r');
plot(cell2mat(sim_learning.hardness_Data{1, 1}),'-.m*', 'LineWidth', 2); hold on
% nominal
plot((1:1:size(cell2mat(sim_learning.agentExp{1, 1}(:,2)))),6.03.*ones(size(cell2mat(sim_learning.agentExp{1, 1}(:,2)))), 'ko'); 
legend('rf', 'rf nominal', 'ore hardness','ore hardness nominal'); hold on; grid on
        ...
        for stepCntr = 1:1:training.nmberOfSteps
            ...
            if State(currentTimeStamp) ~= myEnvironment.Terminal &&...
                ...
                if learning.nxtState_1 ~= myEnvironment.Terminal
                    ...
                    Reps.action_value(learning.crntAgentState_1,learning.crntAgentState_2,...
                            learning.crntAgentState_3,learning.crntAgentAction) = ...
                               ...
                               learning.nxtAgentState_2,learning.nxtAgentState_3,...
                               learning.nxtAgentAction)...
                               -Reps.action_value(learning.crntAgentState_1,...
                               learning.crntAgentState_2,learning.crntAgentState_3,...
                               learning.crntAgentAction));
...
function [nxtModelCoordinate,nxtPSE] = simulateMDP(currentTimeStamp,...
    ...
    % Xmw, Xms, Xmf, Xmr, Xmb, Xsw, Xss, Xsf, IE(t), IAE_PSE,
    % ITAE_PSE
